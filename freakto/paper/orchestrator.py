"""Canonical Freakto automated research-paper cycle orchestrator.
...
from freakto.paper.state_paths import CANONICAL_RELATIVE_DIR, atomic_write_json

VERSION = "1.0.0"
SUCCESS_STATUSES = {
    "COMPLETE",
    "COMPLETE_WITH_MAINTENANCE_WARNINGS",
    CYCLE_NETWORK_SKIPPED,
}
DEFAULT_OUTPUT_DIR = CANONICAL_RELATIVE_DIR
...
def run_step(
    ...
    *,
    ...
    if last.returncode == NETWORK_EXHAUSTED_EXIT_CODE:
        status = STEP_NETWORK_SKIPPED
    else:
...
def cycle_commands(config: OrchestratorConfig, python_executable: str) -> List[tuple[str, List[str], bool]]:
    ...
    commands.extend(
        [
            ...
            (
                "paper_evaluator",
                [python_executable, "-X", "utf8", "-m", "freakto.paper.trade_launch", "--evaluate"],
                False,
            ),
            (
                ...
                [
                    ...
                    "utf8",
                    "-m",
                    "freakto.paper.performance_report",
                    "--output-dir",
            ...
            (
                "paper_status",
                [python_executable, "-X", "utf8", "-m", "freakto.paper.trade_launch", "--status"],
                False,
            ),
        ]
    )
...
class PaperResearchOrchestrator:
    def __init__(
        ...
        *,
        ...
        self.now_fn = now_fn
        worker_token = os.getenv("FREAKTO_CAMPAIGN_WORKER_TOKEN", "")
        self.worker_identity_sha256 = (
            hashlib.sha256(worker_token.encode("utf-8")).hexdigest()
            if worker_token
            else None
        )
    ...
    def _run_commands(self, commands: Iterable[tuple[str, List[str], bool]]) -> List[StepResult]:
        ...
        for name, command, critical in commands:
            ...
            if critical and result.status == STEP_NETWORK_SKIPPED:
                self.logger.warning(
                    "Critical ingestion step %s exhausted all network providers; "
                    "remaining observation steps are skipped.",
                    name,
                )
                break
    ...
    def run_cycle(self, *, force_maintenance: bool = False) -> CycleResult:
        self.cycle_number += 1
        started = self.now_fn()
        cycle_id = started.astimezone(timezone.utc).strftime(
            "paper_cycle_%Y%m%d_%H%M%S"
        )
        ...
        network_skipped = any(
            item.status == STEP_NETWORK_SKIPPED for item in cycle_steps
        )
        maintenance_steps: List[StepResult] = []
        if maintenance and not network_skipped:
        ...
        failed_cycle_steps = [
            ...
            if item.status not in {"PASSED", STEP_NETWORK_SKIPPED}
        ]
        ...
        if network_skipped:
            status = CYCLE_NETWORK_SKIPPED
            warnings.append(
                "All configured market-data providers failed with retryable "
                "network errors; no observation or trade was generated."
            )
        elif failed_cycle_steps:
