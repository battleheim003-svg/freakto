...
from freakto.showcase_paper import showcase_dashboard_data, start_showcase, stop_showcase
...
TEXT = {
    "fa": {
        ...
        "open_positions": "موقعیت باز", "showcase_cards": "کارت‌های آخرین معاملات", "download_card": "دانلود کارت", "showcase_started": "Showcase Paper در پس‌زمینه روشن شد.", "showcase_stopping": "درخواست توقف ثبت شد؛ موقعیت‌ها با قیمت جاری بسته می‌شوند.", "trade_history": "تاریخچه معاملات", "trade_page": "صفحه معاملات",
    ...
    "en": {
        ...
        "open_positions": "Open positions", "showcase_cards": "Latest trade cards", "download_card": "Download card", "showcase_started": "Showcase Paper started in the background.", "showcase_stopping": "Stop requested; positions will close at current prices.", "trade_history": "Trade history", "trade_page": "Trade page",
...
TEXT["en"].update({
    ...
    "demo_auto_refresh": "Paper demo auto-refresh",
    "refresh_interval": "Refresh interval (seconds)",
    "paper_health": "Paper health",
    "state_source": "State source",
    "latest_cycle": "Latest cycle",
    "network_skips": "Network skips",
    "last_heartbeat": "Last heartbeat (UTC)",
})
TEXT["fa"].update({
    "demo_auto_refresh": "به‌روزرسانی خودکار دموی Paper",
    "refresh_interval": "فاصله به‌روزرسانی (ثانیه)",
    "paper_health": "سلامت Paper",
    "state_source": "منبع State",
    "latest_cycle": "آخرین چرخه",
    "network_skips": "چرخه‌های ردشده به‌دلیل شبکه",
    "last_heartbeat": "آخرین Heartbeat (UTC)",
})
...
if st.sidebar.button("↻ " + t("refresh"), use_container_width=True):
    st.session_state["snapshot"] = collect_snapshot(); st.rerun()
default_refresh = validate_refresh_seconds(os.environ.get("FREAKTO_DEMO_REFRESH_SECONDS", 10))
demo_auto_refresh = st.sidebar.toggle(
    t("demo_auto_refresh"),
    value=False,
    key="demo-auto-refresh",
)
demo_refresh_seconds = st.sidebar.number_input(
    t("refresh_interval"),
    min_value=5,
    max_value=60,
    value=default_refresh,
    step=5,
    disabled=not demo_auto_refresh,
    key="demo-refresh-seconds",
)
if demo_auto_refresh:
    components.html(
        "<script>setTimeout(function(){window.parent.location.reload();}, "
        f"{validate_refresh_seconds(demo_refresh_seconds) * 1000});</script>",
        height=0,
    )
st.sidebar.divider()
...
requested_trade_page = max(1, int(st.session_state.get("showcase_trade_page", 1)))
showcase, showcase_trades = showcase_dashboard_data(page=requested_trade_page)
...
if page == "operations":
    ...
    with validation_tab:
        with st.container(border=True):
            ...
            if showcase.get("session_read_warning"):
                st.warning(str(showcase["session_read_warning"]))
            trade_pages = max(1, int(showcase.get("trade_pages", 1) or 1))
            if requested_trade_page > trade_pages:
                requested_trade_page = trade_pages
                st.session_state["showcase_trade_page"] = trade_pages
            selected_trade_page = st.number_input(
                t("trade_page"),
                min_value=1,
                max_value=trade_pages,
                value=min(requested_trade_page, trade_pages),
                step=1,
                key="showcase_trade_page",
            )
            if int(selected_trade_page) != requested_trade_page:
                st.rerun()
            with st.expander(t("trade_history")):
                st.caption(
                    f'{t("trade_page")}: {int(showcase.get("trade_page", 1))}/{trade_pages} '
                    f'· {t("trades")}: {int(showcase.get("total_trades", 0) or 0)}'
                )
                st.dataframe(
                    [
                        {
                            "ID": trade.get("trade_id") or trade.get("source_signal_id"),
                            "Symbol": trade.get("symbol"),
                            "Side": trade.get("side"),
                            "Status": trade.get("status"),
                            "Opened UTC": trade.get("opened_utc"),
                            "Closed UTC": trade.get("closed_utc"),
                            "PnL USDT": trade.get("pnl_usdt"),
                        }
        ...
        campaign = snapshot["paper_demo"]
        with st.container(border=True):
            section_intro(t("campaign"), t("forward_desc"))
            presentation = campaign["status_presentation"]
            metrics([
                (t("paper_health"), campaign.get("health"), f'{presentation["icon"]} {presentation["label"]}', "bad" if presentation["severity"] == "error" else "good" if presentation["severity"] == "success" else ""),
                (t("days"), f'{float(campaign.get("elapsed_days", 0)):.2f}/{campaign.get("minimum_days", 60)}', campaign.get("target_end_utc") or "—", ""),
                (t("cycles"), campaign.get("cycles", 0), campaign.get("campaign_id") or "—", ""),
                (t("network_skips"), campaign.get("network_skipped_cycles", 0), campaign.get("last_cycle_utc"), ""),
            ])
            status_message = f'{presentation["icon"]} {presentation["label"]} — {presentation["description"]}'
            if presentation["severity"] == "error":
                st.error(status_message)
            elif presentation["severity"] == "warning":
                st.warning(status_message)
            elif presentation["severity"] == "success":
                st.success(status_message)
            else:
                st.info(status_message)
            latest_presentation = campaign.get("latest_cycle_presentation")
            if latest_presentation:
                latest_message = (
                    f'{t("latest_cycle")}: {latest_presentation["icon"]} '
                    f'{latest_presentation["label"]} · {campaign.get("last_cycle_utc")}'
                )
                if latest_presentation["severity"] == "error":
                    st.error(latest_message)
                elif latest_presentation["severity"] == "warning":
                    st.warning(latest_message)
                else:
                    st.caption(latest_message)
            status_rows([
                (t("last_heartbeat"), campaign.get("heartbeat_utc")),
                (t("state_source"), campaign.get("persistence_source")),
                (t("trades"), f'{campaign.get("closed_trades", 0)}/{campaign.get("minimum_closed_trades", 200)}'),
            ])
