...
from freakto.showcase_paper.session_reader import (
...
def showcase_dashboard_data(
    ...
    *,
    ...
    view = read_session_view(
        output_dir(root) / "session.json",
        page=page,
        page_size=page_size,
        analytics_limit=DEFAULT_ANALYTICS_LIMIT,
    )
    session = view.metadata
    trades = view.analysis_trades
    state.update(
        open_trades=view.open_trades,
        closed_trades=view.closed_trades,
        total_trades=view.total_trades,
        ...
        session_read_warning=view.warning,
        session_read_degraded=view.degraded,
        session_records_loaded=len(trades),
        trade_page=view.page,
        trade_page_size=view.page_size,
        trade_pages=view.total_pages,
    )
    baseline = int((session.get("session_guard") or {}).get("baseline_closed_trades", 0) or 0)
    omitted = max(0, view.total_trades - len(trades))
    bounded_baseline = max(0, baseline - omitted)
    ...
    maturity_profiles = {
        key: maturity_report(trades, profile_key=key, session_baseline=bounded_baseline)
    ...
    state["performance"] = performance_report(trades, session_baseline=bounded_baseline)
    return state, view.trades


def showcase_status(root: Path = ROOT) -> dict[str, Any]:
    state, _ = showcase_dashboard_data(root, page=1, page_size=1)
    return state


def list_showcase_trades(
    root: Path = ROOT,
    *,
    page: int = 1,
    page_size: int = DEFAULT_ANALYTICS_LIMIT,
) -> list[dict[str, Any]]:
    view = read_session_view(
        output_dir(root) / "session.json",
        page=page,
