...
Evidence Graph مسیر شواهد تا outcome را به صورت node/edge/path ذخیره می‌کند. این خروجی برای بازبینی وزن evidence در آینده استفاده می‌شود، نه برای ورود مستقیم.


## v10.2 Replay Score Calibration

```cmd
python -X utf8 replay_score_calibration_dashboard.py --compact
```

این مرحله Score bands، monotonicity، Feature attribution و Interactionها را با Thresholdهای Train-only روی Validation/Test بررسی می‌کند. خروجی فقط Research/Forward Shadow است و Strategy را خودکار تغییر نمی‌دهد.
