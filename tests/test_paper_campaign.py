...
def test_campaign_start_preflights_arms_and_spawns_safe_loop(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        campaign,
        "_pid_liveness",
        lambda pid: campaign.ProcessLiveness.ALIVE,
    )
...
def test_stop_is_cooperative_and_persistent(monkeypatch, tmp_path):
    ...
    assert (tmp_path / "logs" / "paper_launch_v2" / "campaign_stop.flag").is_file()
...
def test_campaign_recovers_real_windows_worker_pid_from_heartbeat(monkeypatch, tmp_path):
    ...
    assert status["process_identity"] == "UNVERIFIED"


def test_campaign_recovers_only_verified_worker_identity(monkeypatch, tmp_path):
    prepare_policy(tmp_path)
    identity = "a" * 64
    write_state(
        campaign.state_path(tmp_path),
        {
            ...
            "pid": 99,
            "worker_host": campaign.socket.gethostname(),
            "worker_identity_sha256": identity,
        },
    )
    heartbeat = tmp_path / "logs" / "paper_launch_v2"
    heartbeat.mkdir(parents=True)
    (heartbeat / "heartbeat.json").write_text(
        json.dumps(
            {
                "pid": 99,
    ...
    assert status["status"] == "RUNNING"
    assert status["process_liveness"] == "ALIVE"
    assert status["process_identity"] == "VERIFIED"
...
def test_blocked_preflight_never_spawns_worker(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: CommandResult(
            ("python",),
            2,
            json.dumps({"blockers": ["Event universe is missing.", "Holdout benchmarks are missing."]}),
            "",
        ),
    )
    monkeypatch.setattr(campaign.subprocess, "Popen", lambda *args, **kwargs: pytest.fail("must not spawn"))
    with pytest.raises(RuntimeError, match="Event universe is missing"):
