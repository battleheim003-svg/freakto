...
def test_canonical_commands_and_windows_launchers_exist():
    for name in ("start_paper_trading.bat", "run_paper_cycle_once.bat", "stop_paper_trading.bat", "show_paper_status.bat"):
        text = (ROOT / name).read_text(encoding="utf-8")
        assert "freakto.cli" in text
        assert "LIVE_TRADING_ENABLED=true" not in text
    dashboard = (ROOT / "show_paper_dashboard.bat").read_text(encoding="utf-8")
    assert "run_control_center.bat" in dashboard
    assert "[DEPRECATED]" in dashboard
    assert "LIVE_TRADING_ENABLED=true" not in dashboard
