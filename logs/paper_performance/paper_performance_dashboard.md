# Freakto Paper Performance Dashboard

Generated UTC: 2026-07-19T15:23:35.865424+00:00

## Portfolio summary
- Status: **NO_PAPER_TRADES**
- Signals: **0**
- Closed / Open: **0 / 0**
- Wins / Losses: **0 / 0**
- Win rate: **0.00%**
- Profit factor: **0.000**
- Expectancy: **0.0000R**
- Cumulative return: **0.0000R**
- Max drawdown: **0.0000R**
- Best / Worst: **0.0000R / 0.0000R**

## Virtual account (compounded, closed trades only)
- Initial balance: **$10,000.00**
- Current balance: **$10,000.00**
- Total P&L: **$0.00 (0.00%)**
- Risk per trade: **1.00% of current balance**
- Max drawdown: **$0.00 (0.00%)**

## Regime performance

No regime rows yet.

## Equity curve

Closed observations on curve: **0**

This dashboard is research-paper only. It does not enable real orders or capital allocation.