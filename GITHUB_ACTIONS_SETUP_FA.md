...
Workflow `freakto-market-replay.yml` بعد از ساخت Replay، canonical metrics و سپس Score Calibration را اجرا می‌کند. Artifactهای calibration در مسیر `logs/market_replay/calibration/` آپلود می‌شوند. این Workflow هیچ Paper/Live فعال نمی‌کند.
